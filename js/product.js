...
function appendSearchProduct(value = "") {
  if (localStorage.getItem("productTitle") === "สินค้าทั้งหมด") {
    appendProduct(
    ...
    appendProduct(
      ...
      allProduct.filter((e) => {
        let defCondition =
          e.Type === localStorage.getItem("productTitle") ||
          e.MainType === localStorage.getItem("productTitle") ||
          e.Name.toLowerCase().includes(
            localStorage.getItem("productTitle").toLowerCase()
          );
        return value.length
          ? defCondition && e.Name.toLowerCase().includes(value.toLowerCase())
          : defCondition;
...
// Title Name
document.querySelector("#categoryTitle").innerHTML =
  localStorage.getItem("productTitle");
document.title += ` - ${localStorage.getItem("productTitle")}</span>`;
